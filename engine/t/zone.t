...
SKIP: {
    ...
    ok($result_count == 293, "Got $result_count lines from logger.");
    my $info_count = scalar(grep { $_->[2] eq 'INFO' } @res);
    ok($info_count == 64, "$info_count INFO-level results.");
    ...
    ok($debug_count == 223, "$debug_count DEBUG-level results.");
