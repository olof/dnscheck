...
SKIP: {
    ...
    ok($result_count == 273, "Got $result_count lines from logger.");
    my $info_count = scalar(grep { $_->[2] eq 'INFO' } @res);
    ok($info_count == 60, "$info_count INFO-level results.");
    ...
    ok($warning_count == 3, "$warning_count WARNING-level results.");
    ok($dc->logger->count_warning == $warning_count);
    my $debug_count = scalar(grep { $_->[2] eq 'DEBUG' } @res);
    ok($debug_count == 203, "$debug_count DEBUG-level results.");
