...
=head3 The delegation change detector

L<dnscheck-nsdiff> is similar to L<dnscheck-zonediff> in that it downloads a
given zone and trawls through the data. Rather than schedule tests, though, it
just adds rows to the C<delegation_history> table in the database.
...
info given in the L<site_config.yaml> file, you will need to point out a
